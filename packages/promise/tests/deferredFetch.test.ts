...
describe('PromisE.deferredFetch', () => {
	...
	it('should merge headers', async () => {
		const context = getDeferredContext()
		const globalHeadersOrg = config.defaults.fetchOptions.headers
		...
		config.defaults.fetchOptions.headers = globalHeaders
		const getProduct = PromisE.deferredFetch(
			context,
			`${fetchBaseUrl}/1`, // default url
		...
		config.defaults.fetchOptions.headers = globalHeadersOrg
	})
})
