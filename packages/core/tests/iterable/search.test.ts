...
describe('search', () => {
	...
	describe('array', () => {
		it('should return empty array when non-array provided or array with non-object values provided', () => {
			...
			expect(mapSearch([1, 2, 4] as any, config)).toEqual([])
			expect(mapSearch(null as any, config)).toEqual([])
			expect(mapSearch(undefined as any, config)).toEqual([])
		})
		...
		it('should return rearch result as array', () => {
			...
			expect(mapSearch(arr, config)).toEqual(first2)
			...
			expect(mapSearch(arr, config)).toEqual([arr[1]])
		})
	})
})
